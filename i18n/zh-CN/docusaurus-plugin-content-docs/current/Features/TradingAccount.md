...
- 交易账户不是理财账户，没有币本位收益，但是会有手续费点卡奖励；

- 交易账户内的代币会存储在 Mixin 和交易所内，部分代币出于安全考虑，存储在多签钱包内，不会被消耗；

- 交易账户的数据并非实时更新，每五分钟会更新一次数据，如果遇到数据不准时，请耐心等待；

- 部分代币有多种版本，交易账户暂时仅支持其中一种版本，如果您持有某代币但是无法转入交易账户，请查看交易账户支持的版本，通过 ExinOne 的闪兑功能更换版本后存入。
...
**请注意：出于流动性考虑，部分代币（仅入账到交易账户）全部存储与相关交易所，请注意交易所风险。**
...
A：您购买的此类代币主要被存储于交易所内，Mixin 内只有较少的相关资产，如果每次交易都从交易所提币过来，成本将非常巨大，每一次链上的转账大约要消耗 $5，而且链上确认时间也很久，这样会严重影响用户的交易体验。
