...
即：用户可以自由交易相关代币，代币入账到交易账户，您交易所得的代币主要被存储于交易所内，以供您随时交易。
...
- 交易账户内的代币会存储在 Mixin 和交易所内，部分代币出于安全考虑，会存储在多签钱包内，不会被消耗；
...
A：交易账户内的代币存储于 Mixin，交易所和多签钱包内，不会用作它途，ExinOne 有许多措施来保证交易账户内的资产安全。
**请注意：出于流动性考虑，仅入账到交易账户的代币部分或全部存储于相关交易所，请注意交易所风险。**
...
- 基于 ExinOne 的[业务模式](/docs/Instructions/TradingModel)，部分代币流动性不佳，无法提币到 Mixin 钱包；
...
Q：你们不是说资产充足吗，为什么有时候币交易不了，卖不出去？

A：由于 ExinOne 的[业务模式](/docs/Instructions/TradingModel) ，相关资产不一定全部在交易所，如果有部分资产在 Mixin，在您大额交易的时候，可能会出现交易所流动性不足的情况，此时 ExinOne 会通过向交易所借币/将 Mixin 内的资产调拨去交易所等方式进行补充，您可以耐心等待，稍后再试。
